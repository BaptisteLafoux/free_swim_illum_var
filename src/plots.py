...
from utils.loader import dataloader_multiple, dataloader
...
def fig_timeserie_graph(ds, downsampling_period=10):
    ...
    add_illuminance_on_plot(ax, ds, scaling_factor=1, label='$E/E_{\mathrm{max}}$', color='k', linewidth=1, linestyle='--')
    
    ds_cors = ds.coarsen(time=int(downsampling_period*ds.fps), boundary='trim').mean()
    
    add_var_vs_time(abs(ds_cors), 'rot_param', ax, downsmpl_per=2, color='C3', label='$\mathcal{M}$') #warning here : downsampling twice, so real downsampling period is weird
    
    ax.set_xlabel('Time [s]'); ax.set_ylabel('[-]')
    ax.legend()
    
    if SAVE_FIG: fig.savefig('output/subfigures/fig_timeserie_graph.pdf', transparent=True, format='pdf')
...
def fig_timeserie_snapshots(ds):
    ...
    for i, t in enumerate(times):
        ...
        if SAVE_FIG: fig.savefig(f'output/subfigures/snap_{i}.pdf', transparent=True, format='pdf', bbox_inches = 'tight', pad_inches = 0)
...
def fig_general_plot(ds):
    ...
    ###
    add_var_vs_light(abs(ds), 'pol_param', ax[0], correc=True, all_values=False, color='C4', label='Polarization $\mathcal{P}$', marker=None, linestyle='-')
    add_var_vs_light(abs(ds), 'rot_param', ax[0], correc=True, all_values=False, color='C3', label='Milling $\mathcal{M}$', marker=None, linestyle='-')

    ax0 = ax[1]; ax1 = ax0.twinx()
    ...
    if SAVE_FIG: fig.savefig('output/fig_general.pdf', transparent=True, format='pdf')
...
def fig_example_frame(ds, t=3400):
    ...
    if SAVE_FIG: fig.savefig('output/subfigures/frame_example_bg.png', dpi=300)
    ...
    if SAVE_FIG: fig.savefig('output/subfigures/frame_with_arrows.pdf', format='pdf', dpi=300)
